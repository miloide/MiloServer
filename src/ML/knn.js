...
Knn.prototype.getNeighbors = function(test){
    ...
    this.x.forEach(function(item, index){
        if (this.distance == "euclideanDistance"){
            var distance = self.euclideanDistance((item instanceof Array)? item:[item],
                                                  (test instanceof Array)? test:[test]);
        } else {
            var distance = self.manhattanDistance((item instanceof Array)? item:[item],
                                                  (test instanceof Array)? test:[test]);
        }
        var node = {};
...
Knn.prototype.getPrediction = function(test){
    ...
    console.log(nearest);
    var predictedVal = Object.keys(nearest).reduce(function(a, b){
 return nearest[a] > nearest[b] ? a : b;
});
    return predictedVal;
};
