...
var methods = {
    ...
    createTable : function() {
        ...
        for ( var i = 0; i < options.colHeaders.length; i++) {
            ...
            var align = options.colAlignments[i];
            //align = 'center';
    ...
    setData : function(data, ignoreSpare) {
        ...
        for ( var j = 0; j < $.fn.jexcel.defaults[id].data.length; j++) {
            ...
            // Data columns
            for ( var i = 0; i < $.fn.jexcel.defaults[id].colHeaders.length; i++) {
                // New column of data to be append in the line
                var td = $(this).jexcel('createCell', i, j);
        ...
        if ($(cell).hasClass('readonly') == true) {
            ...
            if (options.columns[position[0]].editor) {
                ...
                if (options.columns[position[0]].type == 'checkbox' || options.columns[position[0]].type == 'hidden') {
                    ...
                    // List result
                    var showResult = function(data, str) {
                        // Reset data
                        $(result).html('');
                        // Create options
    ...
    insertColumn : function (numColumns, properties, position, ignoreEvents) {
        ...
        if (options.allowInsertColumn == true) {
            ...
            // Create columns
            for ( var i = num; i < (num + numColumns); i++) {
